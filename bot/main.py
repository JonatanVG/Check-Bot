...
import aiohttp # This is used to make the bot ping itself to avoid idling on hosting platforms like Koyeb.
import asyncio # This is used to make the bot ping itself to avoid idling on hosting platforms like Koyeb.
### End import libraries.

load_dotenv() # Makes the code able to read the .env file.

### Import guild functions
from src.guild_funcs.register_role_with_guild import registered_guilds
from src.guild_funcs.has_correct_roles import has_correct_roles
from src.guild_funcs.guild_owner_only import guild_owner_only
from src.guild_funcs.user_specific_perms import user_specific_perms
### End of import guild functions

### Load environment variables
token = os.getenv("BOT_TOKEN") # Write this to access your bots token.
koyeb_url = os.getenv("KOYEB_URL") # This is used to make the bot ping itself to avoid idling on hosting platforms like Koyeb.
...
import src.roblox_funcs.command_main as command_main
@bot.tree.command(name="bgc_run", description="Runs a background check on the specified user(s).")
@user_specific_perms() # This makes so only users with permissions in the database can run this command.
...
import src.missingFromDatabaseChecker.Comparison as bot_funcs
...
import src.guild_admin_funcs.add_role as add_role_func
...
import src.guild_admin_funcs.remove_role as remove_role_func
...
import src.guild_admin_funcs.show_roles as show_roles_func
...
import src.bot_managment.add_user as add_user_func
...
import src.bot_managment.remove_user as remove_user_func
...
import src.bot_managment.shutdown as shutdown_func
...
import src.bot_managment.show_guilds as show_guilds_func
...
## Server ping: This is used to ping the bot itself to avoid idling on hosting platforms like Koyeb.
@tasks.loop(minutes=15) # This makes the function run every 15 minutes.
async def ping_server():
  try:
    async with aiohttp.ClientSession() as session:
      async with session.get(koyeb_url, timeout=10) as response:
        if response.status == 200:
          print("Successfully pinged the server to prevent idling.")
        else:
          print(f"Failed to ping the server. Status code: {response.status}")
  except asyncio.TimeoutError:
    print("Timeout occurred while trying to ping the server.")
  except Exception as e:
